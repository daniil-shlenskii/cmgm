...
python -u -m main \
  ...
  wandb.name=mgm_critic_sync_smoke \
  callbacks.checkpoint_every_n_steps.every_n_train_steps=1000000 \
  checkpointing.save_dir=/home/ivanov/posterior-refinement/checkpoints/sudoku_mgm_critic_sync_smoke \
  2>&1 | tee /tmp/mgm_critic_sync_smoke.log
