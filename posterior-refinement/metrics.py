...
class Metrics:
    def __init__(self, gen_ppl_eval_model_name_or_path=None,
        ...
        self.is_sudoku = is_sudoku
        if is_sudoku:
            self.sudoku_validity = SudokuValidity()
        # gsm8k accuracy is aggregated across val samples (mean of 0/1).
        # Created unconditionally so the real-GSM8K-test eval is gated only on
        # eval.gsm8k_test_n > 0.
        self.gsm8k_acc = torchmetrics.aggregation.MeanMetric()
        if not is_sudoku:
    ...
    def to(self, *args, **kwargs):
        ...
        if hasattr(self, 'sudoku_validity'):
            self.sudoku_validity = self.sudoku_validity.to(*args, **kwargs)
        if hasattr(self, 'gsm8k_acc'):
            self.gsm8k_acc = self.gsm8k_acc.to(*args, **kwargs)
    ...
    @torch.no_grad()
    def record_sudoku_validity(self, samples):
        if hasattr(self, 'sudoku_validity'):
            self.sudoku_validity.update(samples)
...
class SudokuValidity(torchmetrics.Metric):
    """Computes percentage of valid sudoku solutions.

    A valid sudoku must satisfy ALL constraints:
    - Each row contains digits 1-9 exactly once
    - Each column contains digits 1-9 exactly once
    - Each 3x3 box contains digits 1-9 exactly once

    Input: (B, 81) tensor with values 1-9 (cond_sudoku convention).
    Output: Percentage of valid sudokus in range [0.0, 100.0]
    """

    def __init__(self):
        super().__init__()
        self.add_state("valid_count",
    ...
    def update(self, sudoku_batch: torch.Tensor):
        batch_size = sudoku_batch.shape[0]
        grids = sudoku_batch.cpu().numpy()
        valid_count_batch = 0
        for i in range(batch_size):
            grid = grids[i].reshape(9, 9)
            if self._is_valid_sudoku(grid):
                valid_count_batch += 1
        self.total_count += batch_size
        self.valid_count += valid_count_batch

    def _is_valid_sudoku(self, grid: np.ndarray) -> bool:
        ...
        for col_idx in range(9):
            if set(grid[:, col_idx]) != target_set:
                return False
    ...
    def compute(self) -> torch.Tensor:
        ...
        return 100.0 * self.valid_count.float() / self.total_count.float()
