...
python -u -m main \
  hydra.run.dir=outputs/sflm_sudoku_hard/mgm_fmlminit_adaptive_eval \
  mode=ppl_eval \
  seed=1 \
  ...
  sampling.steps=8 \
  +wandb.offline=true \
  2>&1 | tee /home/ivanov/posterior-refinement/mgm_eval_hard_adaptive.log
