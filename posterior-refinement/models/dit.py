...
class ModulationRMSNorm(nn.Module):
    """RMS-normalize AdaLN modulation outputs.

    Modes:
        - none: pass through unchanged
        - fixed: RMSNorm without learnable scale
        - adaptive: RMSNorm with learnable scale
    """

    def __init__(self, dim, mode='none', eps=1e-6):
        super().__init__()
        self.dim = dim
        ...
        if self.mode not in {'none', 'fixed', 'adaptive'}:
            raise ValueError(
                f"adaln_norm_mode must be one of none/fixed/adaptive, got {mode!r}")
    ...
    def forward(self, x):
        ...
        if self.weight is not None:
            x = x * self.weight.view(*([1] * (x.ndim - 1)), -1)
        return x.to(dtype=orig_dtype)


class SinCosPositionEmbedding1D(nn.Module):
    """Deterministic 1D sin/cos position embedding."""

    def __init__(self, dim, max_len, max_period=10000):
        super().__init__()
        self.dim = dim
        ...
        self.register_buffer(
            'pos_emb',
            self._build_pos_emb(max_len, dim, max_period),
            persistent=False)

    @staticmethod
    def _build_pos_emb(max_len, dim, max_period):
        positions = torch.arange(max_len, dtype=torch.float32)[:, None]
        half = dim // 2
        if half == 0:
            emb = torch.zeros(max_len, dim, dtype=torch.float32)
        else:
            freqs = torch.exp(
                -math.log(max_period)
    ...
    def forward(self, x):
        ...
        pos_emb = self.pos_emb[:, :seq_len].to(device=x.device, dtype=x.dtype)
        return x + pos_emb
...
class DDiTBlock(nn.Module):
    def __init__(self, dim, n_heads, adaLN,
                 ...
                 dropout=0.1, softcap=50):
        super().__init__()
        self.n_heads = n_heads
        self.adaLN = adaLN
        # Attention logit softcap (tanh). Default 50 preserves the value this
        # repo's models (e.g. FMLM+) were trained with. Set 0 to disable —
        # required for checkpoints trained without softcap (e.g. s-flm MDLM).
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int):
        ...
        self.use_pos_emb = bool(getattr(config.model, 'use_pos_emb', False))
        self.vocab_embed = EmbeddingLayer(dim, vocab_size)
        self.pos_embed = (
            SinCosPositionEmbedding1D(dim, self.seq_len)
        ...
        # Optional class-conditional embedding. Backward compatible: only
        # instantiated when model.num_classes is set and > 0; otherwise the
        # model behaves exactly as before.
        num_classes = int(getattr(config.model, 'num_classes', 0) or 0)
        self.num_classes = num_classes
        if num_classes > 0 and not self.causal:
            self.label_embedder = LabelEmbedder(
                num_classes=num_classes, cond_size=cond_dim)
        else:
        ...
        for _ in range(config.model.n_blocks):
            if self.causal:
                ...
                block = DDiTBlock(
                    ...
                    adaLN=self.adaLN,
                    dropout=config.model.dropout,
                    softcap=float(getattr(config.model, 'softcap', 50)))
            blocks.append(block)
